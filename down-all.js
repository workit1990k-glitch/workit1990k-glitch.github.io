// down-allmanga.js - Manga Downloader for allmanga.to
// Loaded via bookmarklet from GitHub Raw
// Version: 2024-03-20 (Adapted for allmanga.to / allanime API)
...
(function() {
  ...
  console.log('📚 Manga Downloader for allmanga.to v2024-03-20 initialized');
  ...
  // GraphQL persisted query hashes (from allanime API)
  const QUERY_HASHES = {
    mangaInfo: '529b0770601c7e04c98566c7b7bb3f75178930ae18b3084592d8af2b591a009f',
    chapterPages: '121996b57011b69386b65ca8fc9e202046fc20bf68b8c8128de0d0e92a681195',
    mangaList: 'a27e57ef5de5bae714db701fb7b5cf57e13d57938fc6256f7d5c70a975d11f3d'
  };
  ...
  // Scanlator/group filter state (adapted for translation groups)
  let selectedGroups = new Set();
  let availableGroups = [];

  // ============ UTILS ============
  const extractCode = (url) => {
    // allmanga.to patterns: /manga/:id, /title/:id, or /:id
    const patterns = [
      /allmanga\.to\/manga\/([^/?#]+)/i,
      /allmanga\.to\/title\/([^/?#]+)/i,
      /allmanga\.to\/([^/?#]+)(?:\/|$)/i
  ...
  const buildGraphQLUrl = (variables, hash) => {
    const extensions = {
      persistedQuery: { version: 1, sha256Hash: hash }
    };
    return `${API_BASE}?variables=${encodeURIComponent(JSON.stringify(variables))}&extensions=${encodeURIComponent(JSON.stringify(extensions))}`;
  };

  const extractGroups = (chapters) => {
    const groups = new Set();
    chapters.forEach(ch => {
      // Try multiple possible fields for group/translator info
      const group = ch.group?.name || 
                   ch.scanlationGroup?.name || 
                   ch.translatedBy || 
                   ch.attributes?.group || 
                   'Unknown';
      groups.add(group);
  ...
  const fetchMangaInfo = async (mangaId) => {
    const url = buildGraphQLUrl({ _id: mangaId }, QUERY_HASHES.mangaInfo);
    const res = await fetchRetry(url);
    return res.data?.manga || res.data?.result || null;
  };

  const fetchAllChapters = async (mangaId) => {
    // Note: allanime API may not have a direct "list all chapters" endpoint
    // We'll fetch manga info which should include chapters, or use pagination
    const chapters = [];
    let page = 1, hasMore = true;
    
    while (hasMore) {
      try {
        // Try to fetch chapters via manga info with pagination
        const variables = {
          _id: mangaId,
          limit: 100,
          page: page,
        ...
        const url = buildGraphQLUrl(variables, QUERY_HASHES.mangaInfo);
        const res = await fetchRetry(url);
        
        const manga = res.data?.manga || res.data?.result;
        const items = manga?.chapters?.edges || manga?.episodes?.edges || [];
        ...
        if (!items.length) {
          ...
          items.forEach(item => {
            const ch = item.node || item;
            chapters.push({
              chapter_id: ch._id || ch.id || ch.chapterString,
              number: ch.number || ch.chapterString || ch.name,
              name: ch.name || ch.title || '',
              scanlation_group: {
                name: ch.group?.name || ch.translatedBy || ch.attributes?.group || 'Unknown'
              },
  ...
  const fetchChapterImages = async (chapterId, chapterNum, showLog = true) => {
    try {
      // Build GraphQL request for chapter pages
      const variables = {
        mangaId: mangaId,
        chapterString: chapterId,
        translationType: 'sub',
        limit: 200, // Get all pages
        offset: 0
      };
      const url = buildGraphQLUrl(variables, QUERY_HASHES.chapterPages);
      
      const res = await fetchRetry(url);
      const edges = res.data?.chapterPages?.edges || res.data?.pages || [];
      
      // Extract image URLs from various possible structures
      let imageUrls = edges.map(edge => {
        const page = edge.node || edge;
        return page.url || page.imageUrl || page.image_url || page;
      }).filter(Boolean);
      ...
      const downloadTasks = imageUrls.map((originalUrl, index) => async () => {
        // Use proxy if available, otherwise direct fetch
    ...
    overlay.innerHTML = `
      <div id="mdx-modal" style="background:#1a1b26;color:#c0caf5;border-radius:12px;border:1px solid #414868;max-width:1000px;width:100%;max-height:95vh;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,.5);display:flex;flex-direction:column;">
        <div id="mdx-header" style="padding:16px 20px;border-bottom:1px solid #414868;display:flex;justify-content:space-between;align-items:center;background:#24283b;flex-shrink:0;flex-wrap:wrap;gap:8px;">
          <strong style="font-size:16px;">📚 Manga Downloader <span style="font-size:12px;color:#7982a9;">(allmanga.to)</span></strong>
      ...
      <style>
        ...
        .mdx-group-chip { display:inline-flex; align-items:center; gap:4px; padding:4px 10px; background:#2f3549; border:1px solid #414868; border-radius:16px; font-size:10px; color:#c0caf5; cursor:pointer; transition:all 0.15s; user-select:none; }
        .mdx-group-chip:hover { background:#414868; }
        .mdx-group-chip.active { background:#e0af68; color:#1a1b26; border-color:#f0bf78; font-weight:600; }
        .mdx-group-chip input { display:none; }
        .mdx-group-count { font-size:9px; color:#7982a9; background:#1a1b26; padding:2px 6px; border-radius:10px; }
  ...
  const renderGroupFilters = () => {
    const container = document.getElementById('mdx-group-filters');
    if (!container) return;
    ...
    availableGroups.forEach(group => {
      ...
      chip.className = 'mdx-group-chip' + (selectedGroups.has(group) ? ' active' : '');
      
      const count = allChapters.filter(ch => 
        (ch.scanlation_group?.name || ch.group?.name || 'Unknown') === group
      ).length;
      
      chip.innerHTML = `
        <input type="checkbox" value="${group}" ${selectedGroups.has(group) ? 'checked' : ''}>
        <span>${group}</span>
        <span class="mdx-group-count">${count}</span>
      `;
    ...
    if (availableGroups.length > 1) {
      ...
      allChip.className = 'mdx-group-chip' + (selectedGroups.size === 0 ? ' active' : '');
  ...
  const isChapterFiltered = (chapter) => {
    ...
    const chapGroup = chapter.scanlation_group?.name || chapter.group?.name || 'Unknown';
    return selectedGroups.has(chapGroup);
  };
  ...
  const renderChapters = () => {
    ...
    allChapters.forEach(ch => {
      ...
      const groupName = ch.scanlation_group?.name || ch.group?.name || 'Unknown';
      info.innerHTML = `
        <div id="mdx-chap-num">Ch.${ch.number}${ch.name ? ' — ' + ch.name : ''}</div>
        <div id="mdx-chap-group">${groupName}</div>
        <div class="mdx-chap-scanlator">🏷️ ${groupName}</div>
      `;
  ...
  const selectUniqueRange = () => {
    ...
    sorted.forEach(ch => {
      if (selectedGroups.size > 0 && !isChapterFiltered(ch)) return;
  ...
  const setupEventListeners = () => {
    ...
    const btnGroupSelect = document.getElementById('mdx-btn-group-select');
    if (btnGroupSelect) {
      btnGroupSelect.addEventListener('click', selectByGroup);
    }
  ...
  const init = async () => {
    ...
    try {
      ...
      if (!code) {
        throw new Error('Not a valid allmanga.to manga page');
      }

      mangaId = code;
      
      const manga = await fetchMangaInfo(mangaId);
      
      if (!manga) {
        throw new Error('Manga not found');
      }
      
      latestChapter = manga.latestChapter || manga.chapters?.[0]?.number || '?';
      
      mangaTitle = (manga.name || manga.title || 'manga')
        .replace(/[^a-z0-9\s]/gi, '')
        .trim()
        .slice(0, 50) || 'manga';
      
      allChapters = await fetchAllChapters(mangaId);
      availableGroups = extractGroups(allChapters);
        ...
        <div id="mdx-chapters">
          <div id="mdx-chap-header">
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
              ...
              <div id="mdx-group-filters" style="display:flex;gap:4px;flex-wrap:wrap;"></div>
              <button id="mdx-btn-group-select" class="mdx-btn mdx-btn-secondary" style="padding:4px 10px;font-size:11px;" title="Select chapters matching chosen groups">
                ✅ Select Filtered
              </button>
      ...
      log(`🏷️ Groups found: ${availableGroups.join(', ') || 'Unknown'}`, 'info');
