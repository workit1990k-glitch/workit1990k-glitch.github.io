...
try {
const selected = allChapters.filter(ch => selectedChapters.has(chapter_id));
...
t.style.cssText = `position:fixed;top:20px;right:20px;background:var(--bg-secondary);border:1px solid var(--success);border-radius:8px;padding:12px 20px;color:var(--text-primary);font-size:13px;z-index:3000;box-shadow:0 4px 20px rgba(0,0,0,.4);animation:slideIn 0.3s ease;`;
...
overlay.innerHTML = `<div id="mdx-modal" style="background:#1a1b26;color:#c0caf5;border-radius:12px;border:1px solid #414868;max-width:1000px;width:100%;max-height:95vh;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,.5);display:flex;flex-direction:column;">
...
<!-- Preview Panel -->
<div id="previewPanel" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.95);z-index:100001;padding:20px;">
    <div style="max-width:1000px;margin:0 auto;height:100%;display:flex;flex-direction:column;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            <h3 style="color:#c0caf5;">Preview</h3>
            <button onclick="window.closePreview()" style="background:#f7768e;color:#1a1b26;border:none;padding:8px 16px;border-radius:6px;cursor:pointer;">Close</button>
        </div>
        <div id="previewLoading" style="text-align:center;padding:40px;color:#7982a9;">Loading...</div>
        <div id="previewError" style="text-align:center;padding:40px;color:#f7768e;display:none;"></div>
        <iframe id="previewFrame" style="flex:1;background:#1a1b26;border:1px solid #414868;border-radius:8px;display:none;"></iframe>
    </div>
</div>

<!-- JSON Modal -->
<div id="jsonModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.9);z-index:100002;padding:20px;">
    <div style="max-width:800px;margin:0 auto;height:100%;display:flex;flex-direction:column;background:#1a1b26;border:1px solid #414868;border-radius:12px;">
        <div style="padding:16px;border-bottom:1px solid #414868;display:flex;justify-content:space-between;align-items:center;">
            <h3 style="color:#c0caf5;margin:0;">JSON Export <span id="fetchingCount" style="font-size:12px;color:#7982a9;"></span></h3>
            <button onclick="window.closeJsonModal()" style="background:none;border:none;color:#7982a9;font-size:20px;cursor:pointer;">&times;</button>
        </div>
        <div style="padding:16px;flex:1;overflow:hidden;display:flex;flex-direction:column;">
            <div id="jsonLoading" style="text-align:center;padding:40px;color:#7982a9;">Fetching Data...</div>
            <div id="jsonError" style="text-align:center;padding:40px;color:#f7768e;display:none;"></div>
            <div id="fetchProgress" style="padding:8px;color:#7982a9;font-size:12px;"></div>
            <textarea id="jsonOutput" style="flex:1;background:#24283b;color:#c0caf5;border:1px solid #414868;border-radius:6px;padding:12px;font-family:monospace;font-size:11px;resize:none;"></textarea>
            <div style="padding:16px;border-top:1px solid #414868;display:flex;justify-content:flex-end;gap:8px;">
                <span id="copyStatus" style="color:#9ece6a;font-size:12px;display:none;align-self:center;">Copied!</span>
                <button id="copyJsonBtn" onclick="window.copyJsonToClipboard()" disabled style="padding:8px 16px;background:#2f3549;color:#c0caf5;border:1px solid #414868;border-radius:6px;cursor:pointer;">Copy JSON</button>
            </div>
        </div>
    </div>
</div>
...
#chaptersList{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:8px;}
.chapter-item{display:flex;align-items:center;padding:10px 12px;background:#24283b;border-radius:6px;font-size:12px;cursor:pointer;transition:background.15s;border:1px solid transparent;margin-bottom:8px;}
.chapter-item:hover{background:#2f3549;}
.chapter-item.selected{background:rgba(224,175,104,.15);border-color:#e0af68;}
.status-box{width:10px;height:10px;border-radius:50%;margin-right:8px;}
.status-box.green{background:#9ece6a;}
.status-box.yellow{background:#e0af68;}
.status-box.red{background:#f7768e;}
.status-box.fetching{background:#7aa2f7;animation:mdx-spin 1s linear infinite;}
.chapter-info{flex:1;}
.chapter-number{font-weight:600;color:#c0caf5;margin-bottom:2px;}
.chapter-group{font-size:10px;color:#7982a9;}
.chapter-group.official{color:#9ece6a;}
.chapter-actions{display:flex;align-items:center;gap:4px;}
.btn{padding:6px 12px;border:none;border-radius:4px;font-size:11px;cursor:pointer;margin-left:4px;}
.btn-secondary{background:#2f3549;color:#c0caf5;border:1px solid #414868;}
...
#downloadConsole{position:fixed;bottom:20px;right:20px;width:400px;background:#24283b;border:1px solid #414868;border-radius:8px;padding:12px;display:none;flex-direction:column;max-height:280px;z-index:100000;}
#downloadConsole.active{display:flex;}
#consoleLog{flex:1;overflow-y:auto;font-family:monospace;font-size:11px;margin-bottom:8px;}
.console-line{padding:3px 6px;margin:2px 0;border-radius:4px;}
.console-line.info{color:#7982a9;}
.console-line.success{color:#9ece6a;background:rgba(158,206,106,.1);}
.console-line.error{color:#f7768e;background:rgba(247,118,142,.1);}
.console-line.warning{color:#e0af68;background:rgba(224,175,104,.1);}
#consoleProgress{height:4px;background:#1a1b26;border-radius:2px;overflow:hidden;}
#consoleProgressFill{height:100%;background:#9ece6a;transition:width.3s;}
#consoleStats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:8px;font-size:10px;color:#7982a9;}
</style>`;
...
content.innerHTML = `<div id="mdx-chapters">
...
<div id="chaptersCount" style="text-align:center;font-size:11px;color:#7982a9;margin-top:8px;"></div>
</div>
<div id="mdx-footer">
<div id="mdx-actions">
    <button id="mdx-btn" class="mdx-btn-secondary" onclick="window.selectAll()">All</button>
    <button id="mdx-btn" class="mdx-btn-secondary" onclick="window.deselectAll()">None</button>
    <button id="mdx-btn" class="mdx-btn-secondary" onclick="window.selectUniqueChapters()">Unique</button>
    <button id="batchFetchBtn" class="mdx-btn-secondary" onclick="window.startBatchFetch()" disabled>⬇ Fetch</button>
    <button id="batchRefetchFailedBtn" class="mdx-btn-secondary" onclick="window.startBatchRefetchFailed()" disabled>🔄 Refetch</button>
    <button id="showJsonBtn" class="mdx-btn-secondary" onclick="window.showJsonSelected()" disabled>JSON</button>
</div>
<div style="display:flex;align-items:center;gap:16px;">
    <div id="mdx-count"><strong id="selectedCount">0</strong> selected</div>
    <button id="batchZipBtn" style="padding:10px 20px;background:#9ece6a;color:#1a1b26;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;" onclick="window.startBatchZipDownload()" disabled>📦 DOWNLOAD</button>
</div>
</div>`;
...
consoleEl.id = 'downloadConsole'; 
consoleEl.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong style="font-size:12px;color:#c0caf5;">Download Console</strong><div style="display:flex;gap:4px;"><button onclick="window.clearConsole()" style="background:none;border:none;color:#7982a9;cursor:pointer;font-size:10px;">Clear</button><button onclick="window.closeConsole()" style="background:none;border:none;color:#7982a9;cursor:pointer;font-size:16px;">&times;</button></div></div><div id="consoleLog"></div><div id="consoleStats" class="console-stats"><div>Chapters: <span id="statChapters">0/0</span></div><div>Images: <span id="statImages">0</span></div><div>Size: <span id="statSize">0.0 MB</span></div></div><div id="consoleProgress"><div id="consoleProgressFill" style="width:0%"></div></div><div id="batchSummary" style="display:none;margin-top:8px;padding:8px;background:rgba(158,206,106,.1);border-radius:4px;font-size:11px;color:#9ece6a;text-align:center;" class="active"><span id="summaryText"></span><button onclick="window.startBatchZipDownload()" style="margin-left:8px;padding:2px 8px;background:#9ece6a;color:#1a1b26;border:none;border-radius:4px;cursor:pointer;font-size:10px;">Download Now</button></div><div id="pauseBtn" style="display:none;margin-top:8px;"><button onclick="window.togglePause()" style="width:100%;padding:8px;background:#2f3549;color:#c0caf5;border:1px solid #414868;border-radius:4px;cursor:pointer;font-size:11px;">⏸ PAUSE</button></div>'; 
