...
(function() {
  ...
  const fetchChapterImages = async (chapterId) => {
    try {
      ...
      else if (data?.result) imageUrls = fetchChapterImages(data.result);
      
      if (!imageUrls.length) return { blobs: [], size: 0, total: 0, failed: 0 };
      
      const blobs = [];
      let totalSize = 0, failedCount = 0;
      for (let i = 0; i < imageUrls.length; i++) {
        let url = imageUrls[i];
        // 🔥 Use wsrv.nl for resized WebP
  ...
  const renderChapters = () => {
    ...
    allChapters.forEach(ch => {
      ...
      item.onclick = (e) => { if (e.target.type !== 'checkbox') toggleChapter(ch.chapter_id); };
      item.innerHTML = `<input type="checkbox" id="mdx-chap-check" ${isSelected ? 'checked' : ''} onclick="event.stopPropagation();toggleChapter('${ch.chapter_id}')"><div id="mdx-chap-info"><div id="mdx-chap-num">Ch.${ch.number}${ch.name ? ' — ' + ch.name : ''}</div><div id="mdx-chap-group">${ch.scanlation_group?.name || 'Unknown'}</div></div>`;
  ...
  const selectAll = () => { allChapters.forEach(ch => selectedChapters.add(ch.chapter_id)); renderChapters(); updateCount(); };
  const deselectAll = () => { selectedChapters.clear(); renderChapters(); updateCount(); };
  ...
  const selectUnique = () => {
    ...
    allChapters.forEach(ch => {
      ...
      if (!isNaN(num)) {
        ...
        if (!seen.has(intNum)) { seen.set(intNum, ch.chapter_id); selectedChapters.add(ch.chapter_id); }
      }
    });
    renderChapters();
    updateCount();
  ...
  const downloadSelected = async () => {
    ...
    const saveZip = async (isFinal) => {
      ...
      // 🔥 ZIP filename with spaces, no underscores
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        try {
          ...
          if (data.blobs.length > 0) {
            ...
            log('✓ Fetched ' + data.total + ' images (' + (data.size / 1024 / 1024).toFixed(1) + 'MB)', 'success');
          } else {
        ...
        if (currentZipSize >= MAX_ZIP_SIZE && i < selected.length - 1) await saveZip(false);
  ...
  const init = async () => {
    ...
    try {
      ...
      // 🔥 Keep spaces in title for ZIP filename
      mangaTitle = (manga.title || 'manga').replace(/[^a-z0-9\s]/gi, '').trim().slice(0, 50);
      ...
      content.innerHTML = `
        <div id="mdx-chapters">
          <div id="mdx-chap-header"><span>📖 Select Chapters (${allChapters.length} total)</span><span style="font-size:11px;color:#7982a9;">Click to toggle</span></div>
          <div id="mdx-chap-list"></div>
        </div>
        <div id="mdx-footer">
          <div id="mdx-actions">
            <button id="mdx-btn" class="mdx-btn-secondary" onclick="selectAll()">All</button>
            <button id="mdx-btn" class="mdx-btn-secondary" onclick="deselectAll()">None</button>
            <button id="mdx-btn" class="mdx-btn-secondary" onclick="selectUnique()">Unique Only</button>
          </div>
          <div style="display:flex;align-items:center;gap:16px;">
            <div id="mdx-count"><strong id="mdx-selected-count">0</strong> selected</div>
            <button id="mdx-download-btn" style="padding:10px 20px;background:#9ece6a;color:#1a1b26;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;" onclick="downloadSelected()" disabled>📦 DOWNLOAD</button>
          </div>
        </div>
      `;
